import { createSlice, createAsyncThunk } from "@reduxjs/toolkit";
   ...
   export const createPet = createAsyncThunk("pets/create", async (petData, { rejectWithValue }) => {
     try {
       ...
       return response.data; // Возвращаем {message, pet_id}
     } catch (error) {
   ...
   export const updatePet = createAsyncThunk("pets/update", async ({ id, petData }, { rejectWithValue }) => {
     try {
       ...
       return response.data.pet; // Бэкенд возвращает {message, pet}
     } catch (error) {
     ...
     extraReducers: (builder) => {
       builder
         ...
         .addCase(createPet.fulfilled, (state, action) => {
           state.loading = false;
           state.pets.push({ id: action.payload.pet_id, ...action.payload }); // Добавляем pet_id
